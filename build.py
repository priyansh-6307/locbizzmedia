"""Build the LOCBIZZ static site. Python standard library; no framework required."""
...
icon='<svg class="monogram" viewBox="0 0 30 32" fill="none" aria-hidden="true"><rect x="2" y="2" width="26" height="28" rx="6" stroke="currentColor" stroke-width="2.5"/><path d="M9 8v16h12M19 10h3v9h-7" stroke="currentColor" stroke-width="2.5" stroke-linejoin="round"/></svg>'
wordmark='''<svg class="wordmark" viewBox="0 0 414 196" role="img" aria-label="LOCBIZZ"><path d="M151 30V17Q151 7 162 7H249Q260 7 260 17V30" fill="none" stroke="currentColor" stroke-width="12"/><rect x="4" y="42" width="406" height="142" rx="30" fill="none" stroke="currentColor" stroke-width="6"/><path d="M35 185v5h32v-5M348 185v5h32v-5" fill="none" stroke="currentColor" stroke-width="8" stroke-linejoin="round"/><text x="207" y="162" text-anchor="middle" textLength="374" lengthAdjust="spacingAndGlyphs" font-family="Anton, Impact, sans-serif" font-size="112" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linejoin="round">LOCBIZZ</text></svg>'''
...
def footer():
    return f'''<footer class="footer shell"><div class="footer-top"><a href="/" aria-label="LOCBIZZ home">{icon}</a><a class="email-link" href="mailto:{email}">EMAIL ↗</a></div><div class="footer-main"><h4 class="footer-intro">Have a project in mind?<br>We'd love to hear from you.</h4>{form()}</div><div class="footer-bottom"><nav aria-label="Footer navigation"><a href="/projects/">Projects</a><a href="/about/">About</a><a href="/contact/">Contact</a></nav><p>( ©26 LOCBIZZ )</p></div></footer>'''

def page(content,title='LOCBIZZ — CGI, VFX & AI Studio',path='/',kind='',show_footer=True):
    ...
    return f'''<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{e(title)}</title><meta name="description" content="LOCBIZZ creative studio. Cinematic CGI, VFX and AI content for modern brands."><meta name="theme-color" content="#000000"><link rel="icon" type="image/svg+xml" href="/favicon.svg"><link rel="canonical" href="https://locbizz-studio.green-box-3711.chatgpt.site{path}"><link rel="preload" href="/assets/inter-display-medium.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="/style.css"><script src="/app.js" defer></script></head><body class="{kind}"><a class="skip" href="#main">Skip to content</a>
    <header class="topbar"><a href="/" aria-label="LOCBIZZ home">{icon}</a><nav class="toplinks" aria-label="Primary navigation">{nav}</nav><button class="menu-toggle" aria-label="Open menu" aria-expanded="false" aria-controls="menu"><span></span><span></span></button></header>
    <div class="menu" id="menu" hidden><nav aria-label="Menu">{links}</nav><div class="menu-bottom"><a class="menu-email" href="mailto:{email}">{email}</a><span class="mono">LOCBIZZ</span></div></div>
...
write('/',home,'LOCBIZZ — CGI, VFX & AI Studio','home')
...
write('/projects/',f'<div class="shell"><div class="page-heading projects-heading reveal"><h1>Projects.</h1><nav class="filters" aria-label="Project categories">{filters}</nav></div><section class="work"><p class="filter-count" role="status" aria-live="polite">12 projects</p>{grid(projects)}</section></div>','Projects — LOCBIZZ','projects-page')
about='''<section class="about-hero"><video src="https://framerusercontent.com/assets/AGFM8JWT8BuczHUVcPxzdzqiqk.mp4" poster="/assets/about-poster.jpg" muted loop playsinline preload="metadata" aria-hidden="true"></video><h1>We’re LOCBIZZ — a creative studio blending CGI, AI, and emerging technology to create next-generation visual content.</h1></section><section class="about-story shell">'''+icon+'''<h4>Filmmaking is at the heart of our approach. We bring together cinematic storytelling, 3D craft, and new production tools to turn ambitious ideas into images that move people.<br><br>From automotive films to product worlds, we explore what’s possible at the meeting point of imagination and technology.</h4></section>'''
write('/about/',about,'About — LOCBIZZ')
write('/contact/',f'<section class="contact-page shell"><div class="contact-heading reveal"><h1>Got a project in mind? Let’s chat and bring it to life.</h1></div><div class="contact-body"><div><h4>Get in touch</h4><a class="email-link" href="mailto:{email}">{email}</a></div>{form(True)}</div></section>','Contact — LOCBIZZ',show_footer=False)
...
for i,p in enumerate(projects):
    ...
    write('/projects/'+p['slug']+'/',body,p['title']+' — LOCBIZZ')
...
write('/blog/',blog_body,'Creative Dispatch — LOCBIZZ')
for b in blogs:
    ...
    write(b['path']+'/',article,b['title']+' — LOCBIZZ')
...
(OUT/'404.html').write_text(page('<section class="contact-page shell"><h1>Page not found.</h1><p style="margin-top:35px"><a class="modal-link" href="/">BACK TO LOCBIZZ →</a></p></section>','Page not found — LOCBIZZ',show_footer=False),encoding='utf-8')
